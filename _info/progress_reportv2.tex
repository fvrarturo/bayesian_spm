...
We compare \textbf{seven inference methods}---two MCMC samplers (NUTS and a model-specific Gibbs sampler), two variational methods (mean-field and low-rank ADVI), and three frequentist baselines (graphical lasso, Ledoit--Wolf, sample covariance)---across a systematic grid of synthetic experiments varying dimension $p \in \{10, 50, 100\}$, concentration ratio $\gamma \in \{0.10, \ldots, 0.90\}$, and sparsity level $s \in \{0.05, 0.10, 0.30\}$.

Our preliminary findings reveal a \textbf{dimension- and method-dependent failure mode}. At $p{=}50$, mean-field ADVI preserves the horseshoe's bimodal shrinkage with near-nominal coverage (0.96), while low-rank ADVI---counterintuitively---already fails calibration (coverage 0.39). At $p{=}100$, both variational methods collapse (coverage 0.01 / 0.10), and NUTS fails to mix in a deeper, geometric sense: 0 of 50 successful runs meet the strict convergence gate, with median $\hat{R} = 5.0$ and median bulk-ESS $= 2$ (\S\ref{sec:nuts-failure}). The model-specific Gibbs sampler of Li et al.~\cite{li2019}---with hand-derived conditionals and 100\% acceptance---is the unique Bayesian survivor at $p{=}100$: it actually \emph{outperforms} NUTS on Stein's loss at this dimension (2.86 vs.\ 3.00 at $\gamma{=}0.20$, up to 2$\times$ better at $\gamma{=}0.10$) because NUTS's non-mixed chains corrupt its posterior mean. Gibbs also maintains nominal coverage (0.99), preserves the bimodal shrinkage signature ($b = 0.90$), and converges on a single chain in ${\sim}45$~min/seed---8$\times$ faster than NUTS's 7-hour non-convergent runs.
...
\paragraph{NUTS vs.\ Gibbs.} This comparison tests the tradeoff from Lecture~8: a \emph{generic} gradient sampler versus a \emph{model-specific} sampler with hand-derived coordinate updates. Both target the same posterior, but they fail differently as $p$ grows.
...
\paragraph{ADVI low-rank.} To partially capture posterior correlations we fit $q(\bm{\zeta}) = \Normal(\bm{\mu},\; \bm{L}\bm{L}^\top + \bm{D})$ with $\bm{L}\in\R^{D\times r}$, $r = \min(\max(p/2, 25), 100)$ (so $r{=}25$ at $p{\in}\{10,50\}$, $r{=}50$ at $p{=}100$). This gives ${\sim}500\text{K}$ parameters at $p{=}100$, versus ${\sim}50\text{M}$ for full-rank (infeasible).
...
This yields 42 configurations $\times$ 7 methods, with 3{,}329 successful inference runs across all seeds (the Bayesian methods have some $p{=}100$ failures; see Table~\ref{tab:summary}). All computation was performed on the MIT Engaging cluster using NumPyro/JAX (Bayesian methods) and scikit-learn (frequentist baselines).
...
Getting the graphical horseshoe to run reliably at scale required solving engineering problems that are instructive for practitioners working with heavy-tailed priors.
...
\emph{At $p = 10$} (left panel), all methods are clustered between 0.2 and 5---the problem is easy enough that even the sample covariance performs acceptably. NUTS achieves the lowest loss ($\sim$0.15 at $\gamma = 0.10$), but the gap to ADVI-MF and glasso is small.
...
\end{table}
Table~\ref{tab:coverage} shows the calibration story. Both MCMC methods post near-nominal coverage throughout (0.98--1.00), but only Gibbs does so on verified-converged chains: NUTS's $p{=}100$ value comes from non-converged chains that happen to produce wide credible intervals---a symptom of inflated variance, not genuine calibration. Gibbs's $p{=}100$ coverage of 0.99 is thus the unique reliable uncertainty quantification at high dimension. The two variational methods degrade very differently:
...
Final report draft (12 pages, ICML) & May 1 & Federico \\
\midrule
...
\paragraph{Real data.} CRSP daily returns ($p{=}50$ most liquid large-caps, $T{=}250$). Since $\bOmega_0$ is unknown, evaluation shifts to out-of-sample log-likelihood and minimum-variance portfolio performance.
